...
if __name__ == "__main__":
    ...
    x_test = torch.Tensor(torch.linspace(lower,upper,n_samples_gt))
    y_gt = torch.sigmoid(x_test)
    ...
    n_samples_rand = 50
    x_train = lower + (upper-lower)*torch.rand((n_samples_rand))
    prob_rand = torch.sigmoid(x_train)
    y_train = torch.bernoulli(prob_rand)
    x_true = x_train[torch.nonzero(y_train)].squeeze()
    x_false = x_train[torch.nonzero(y_train == 0)].squeeze()
    ...
    fig, ax = plt.subplots() 
    ax.plot(x_test.cpu().numpy(),y_gt.cpu().numpy(),'--k')
    ...
    ax.plot(x_test,p_test)
    ax.fill_between(x_test,p_test-sigma_test,p_test+sigma_test,alpha=0.2)
